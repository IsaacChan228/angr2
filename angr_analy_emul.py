...
# python angr_analy_emul.py <binary_file>

import angr
import angr.sim_options
...
def load_and_analyze_binary(binary_path, verbose=False):
    ...
    try:
        ...
        # Generate control flow graph using emulation
        print("[+] Analysis using CFGemulation...")
        #cfg = project.analyses.CFGEmulated(keep_state=True, state_add_options=angr.sim_options.refs)
        cfg = project.analyses.CFGEmulated(
            keep_state=True,
            call_depth=20,           # limit execution depth
            max_steps=5000,         # limit execution steps
            normalize=True,         # normalize addresses
            resolve_indirect_jumps=False,  # do not resolve indirect jumps
        )
...
def main():
    ...
    print("=" * 60)
    print("angr Binary Analysis Tool (CFGEmulation Version)")
    print("=" * 60)
